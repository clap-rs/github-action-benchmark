...
function buildAnnotation(benchName, curSuite, prevSuite) {
    const lines = [
        `Benchmark ${benchName}`,
        '',
        `Current: ${curSuite.commit.id} vs Previous: ${prevSuite.commit.id}`,
        '',
    ];
    const benches = [];
    for (const current of curSuite.benches) {
        ...
        if (prev) {
            ...
            benches.push([current.name, annVal(current), annVal(prev), floatStr(ratio)]);
        }
        else {
            benches.push([current.name, annVal(current), '', '']);
        }
    }
    const size = [0, 0, 0, 0];
    benches.forEach(row => {
        size[0] = Math.max(row[0].length, size[0]);
        size[1] = Math.max(row[1].length, size[1]);
        size[2] = Math.max(row[2].length, size[2]);
        size[3] = Math.max(row[3].length, size[3]);
    });
    return lines
        .concat(benches.map(row => {
        const s = `${row[0].padEnd(size[0])} | ${row[1].padEnd(size[1])} | `;
        return s + `${row[2].padEnd(size[2])} | ${row[3].padEnd(size[3])}`;
    }))
        .join('\n');
}
...
function handleAnnotation(benchName, curSuite, prevSuite, config) {
    const { annotateAlways } = config;
    if (!annotateAlways) {
        core.debug('Annotation was skipped because annotate-always is disabled');
        return;
    }
    core.debug('Annotating about benchmark comparison');
    const body = buildAnnotation(benchName, curSuite, prevSuite);
    core.warning(body);
}
...
async function writeBenchmark(bench, config) {
    ...
    if (prevBench === null) {
        ...
        handleAnnotation(name, bench, prevBench, config);
